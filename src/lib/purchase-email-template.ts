...
export function purchaseEmail(
  ...
  const button = (href: string, label: string) =>
    `<a href="${escape(href)}" style="display:inline-block;margin:12px 8px 0 0;padding:12px 16px;background:#6695ef;border:1px solid #6695ef;border-radius:6px;color:#0b1528;text-decoration:none;font-size:12px;font-weight:bold;text-align:center;min-width:120px">${label}</a>`;
  const rows = items
    .map(
      (item) =>
        `<tr><td style="padding:16px;border-bottom:1px solid #424448"><strong>${escape(item.title)}</strong><br><span style="font-size:12px">${escape(item.license)}</span>${token && item.itemId ? `<br>${button(directUrl(item, "zip"), "DOWNLOAD ZIP")}${button(directUrl(item, "license"), "DOWNLOAD LICENSE")}` : ""}</td></tr>`,
    )
    .join("");
  return {
