...
export function purchaseEmail(
  ...
  const button = (href: string, label: string, secondary = false) =>
    `<a href="${escape(href)}" style="display:block;padding:15px 5px;background:${secondary ? "#1c2532" : "#6695ef"};border:1px solid ${secondary ? "#6d85a7" : "#6695ef"};border-radius:5px;color:${secondary ? "#e2e7f4" : "#0b1528"};text-decoration:none;font-size:11px;font-weight:bold;text-align:center">${label}</a>`;
  const rows = items
    .map(
      (item) =>
