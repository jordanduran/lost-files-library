...
export function emailReady() {
  return (
    process.env.PURCHASE_EMAIL_ENABLED === "true" &&
    Boolean(
...
export async function deliverPurchaseEmail(orderId: string) {
  ...
  const { data: jobs, error } = await db.rpc("claim_purchase_email", {
    p_order: orderId,
  });
  ...
  try {
    ...
    if (!payload) {
      const { data: order, error: orderError } = await db
        .from("orders")
        .select("is_test,order_items(id,product_title,license_name)")
        .eq("id", orderId)
        .eq("status", "paid")
        .single();
    ...
    if (typeof result.id !== "string")
      throw new Error("Email response invalid");
    const { error: saveError } = await db
      .from("purchase_emails")
      .update({
        status: "sent",
