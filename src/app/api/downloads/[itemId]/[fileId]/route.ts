...
export async function POST(request: Request, context: { params: Promise<{ itemId: string; fileId: string }> }) {
  if (request.headers.get("sec-fetch-site") === "cross-site") return Response.json({ error: "Forbidden" }, { status: 403, headers });
  const user = await getUser();
  if (!user) return Response.json({ error: "Sign in required" }, { status: 401, headers });
  const { itemId, fileId } = await context.params;
  if (![itemId, fileId].every(id => /^[0-9a-f-]{36}$/i.test(id))) return Response.json({ error: "Not found" }, { status: 404, headers });
  try {
    const purchase = await purchasedFiles(user.id, itemId);
    if (!purchase?.files.some(file => file.id === fileId)) return Response.json({ error: "Not found" }, { status: 404, headers });
    const db = adminDatabase();
    const { data: file, error } = await db.from("product_files").select("bucket,object_key,download_name")
      .eq("id", fileId).eq("storage_provider", "supabase").eq("bucket", process.env.DOWNLOAD_BUCKET || "lost-files-demo").single();
  ...
  } catch { return Response.json({ error: "Download unavailable" }, { status: 503, headers }); }
}
