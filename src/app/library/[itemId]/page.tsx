...
export default async function DownloadPage({
  ...
  if (purchase.orderId) {
    const { data: access } = await adminDatabase()
      .from("order_access")
      .select("token")
  ...
  return (
    <div className="page-width cart-page">
      <div className="page-intro">
        <span className="eyebrow">YOUR PURCHASE / DOWNLOADS</span>
        <h1>{purchase.title}</h1>
        <p>{purchase.license}</p>
      </div>
      {purchase.isTest && (
        <p className="sample-note">
          These are synthetic demo WAVs for testing downloads. Final MP3, WAV,
          and stem packages will replace them before launch.
        </p>
      )}
      {purchase.files.length ? (
        <div className="purchase-files">
          {purchase.files.map((file) => (
            <div className="purchase-file" key={file.id}>
              <div>
                <strong>{file.download_name}</strong>
                <p>{(Number(file.size_bytes) / 1024 / 1024).toFixed(1)} MB</p>
              </div>
        ...
        <p role="status">
          Your files are being prepared. Please check back shortly.
        </p>
      )}
      <p>
