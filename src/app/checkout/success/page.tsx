...
export default async function CheckoutSuccess({
  ...
  if (query.session_id) {
    if (!/^cs_(test|live)_[a-zA-Z0-9]+$/.test(query.session_id)) notFound();
    const test = query.session_id.startsWith("cs_test_");
    if (!paymentCredentials(test)) notFound();
    const session = await stripeClient(test)
      .checkout.sessions.retrieve(query.session_id)
      .catch(() => null);
    if (
      !session ||
      session.livemode !== !test ||
      session.metadata?.checkout_kind !== "pack" ||
    ...
    const { data: order } = await db
      .from("orders")
      .select("id,status,is_test,checkout_session_id")
  ...
  const { data: order, error } = await client!
    .from("orders")
    .select("id,status,order_items(product_id,license_id)")
    .eq("id", id)
    .eq("user_id", user.id)
    .single();
