...
export async function setCityVote(city: string, enabled: boolean) {
  if (!CITIES.some(value => value.name === city) || typeof enabled !== "boolean") return {error:"Choose a city from the atlas."};
  try {
    const client=await createClient();
    if (!client) return {error:"Voting is temporarily unavailable."};
    const {data:{user}}=await client.auth.getUser();
    if (!user) return {error:"Sign in to save your votes.",needsLogin:true};
    ...
    if (ready.error) return {error:"Voting is temporarily unavailable. Please try again soon."};
    const {error}=enabled
      ? await client.from("producer_city_votes").upsert({user_id:user.id,city},{onConflict:"user_id,city",ignoreDuplicates:true})
      : await client.from("producer_city_votes").delete().eq("user_id",user.id).eq("city",city);
