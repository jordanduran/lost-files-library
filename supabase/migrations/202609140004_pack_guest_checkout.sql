...
create function public.create_pack_order(p_user uuid, p_request uuid, p_token text, p_product text)
returns uuid language plpgsql set search_path = '' as $$
declare result uuid; product public.products; license public.product_licenses;
begin
  ...
  select a.order_id into result from public.order_access a join public.orders o on o.id=a.order_id
    ...
    and exists(select 1 from public.order_items i where i.order_id=o.id and i.product_id=p_product);
  if result is not null then return result; end if;
  select * into product from public.products where id=p_product and published and kind='pack' and cardinality(pack_tracks)>1;
  select * into license from public.product_licenses where product_id=product.id and id='pack';
  if product.id is null or license.id is null or license.price_cents<=0 or not exists(
    select 1 from public.product_files where product_id=product.id and license_id=license.id
  ...
  insert into public.orders(user_id,request_id,total_cents,is_test) values(p_user,p_request,license.price_cents,true) returning id into result;
