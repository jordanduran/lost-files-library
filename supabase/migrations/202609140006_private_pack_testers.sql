begin;
alter table public.products add column test_restricted boolean not null default false;
-- Private test packs cannot enter the public or legacy beat checkout catalog.
alter table public.products add constraint private_test_pack_unpublished check (not test_restricted or (not published and kind='pack'));
-- A multi-track pack has no single measured tempo.
alter table public.products alter column bpm drop not null;
alter table public.products add constraint beat_requires_tempo check (kind='pack' or bpm is not null);
alter table public.orders add column test_product_ids text[] not null default '{}';
create table public.pack_testers (
...
alter table public.pack_testers enable row level security;
revoke all on public.pack_testers from public,anon,authenticated;
grant all on public.pack_testers to service_role;

create function public.is_pack_tester(p_user uuid,p_product text)
returns boolean language sql stable set search_path='' as $$
  select exists(select 1 from auth.users u join public.pack_testers t on t.email=lower(u.email)
    where u.id=p_user and u.email_confirmed_at is not null and t.product_id=p_product and t.enabled);
$$;
create function public.tester_pack_ids(p_user uuid)
returns table(product_id text) language sql stable set search_path='' as $$
  select t.product_id from public.pack_testers t join auth.users u on t.email=lower(u.email)
    where u.id=p_user and u.email_confirmed_at is not null and t.enabled;
$$;
create function public.test_order_allowed(p_order uuid)
returns boolean language sql stable set search_path='' as $$
  select coalesce((select cardinality(o.test_product_ids)=0 or (o.is_test and not exists(
    select 1 from unnest(o.test_product_ids) p where not public.is_pack_tester(o.user_id,p)
  )) from public.orders o where o.id=p_order),false);
$$;
revoke all on function public.is_pack_tester(uuid,text),public.tester_pack_ids(uuid),public.test_order_allowed(uuid) from public,anon,authenticated;
grant execute on function public.is_pack_tester(uuid,text),public.tester_pack_ids(uuid),public.test_order_allowed(uuid) to service_role;

create or replace function public.create_pack_order(p_user uuid, p_request uuid, p_token text, p_products text[])
...
create or replace function public.confirm_pack_order(p_order uuid,p_session text,p_total integer,p_currency text,p_email text)
...
begin
  ...
  if cardinality(purchase.test_product_ids)>0 then
    if not public.test_order_allowed(p_order) then raise exception 'Tester access required'; end if;
    -- Never send unreleased files to an address typed into Stripe checkout.
