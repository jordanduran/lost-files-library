...
Loading uses dark card skeletons on Library and Packs, and a thin ring for route changes and pending checkout, authentication, recovery, download, and admin-save actions. Indicators appear only during actual waits. Rings are decorative beside readable labels; skeletons expose a named busy status with their placeholder content hidden from assistive technology. Reduced motion makes both static. `/preview/loading` keeps both visible for design review and does not run real transactions.
